...
rm blockly_compressed_horizontal.js 2> /dev/null
echo Compiling Scratch-Blocks..
java -jar $COMPILER \
  --js='core/**.js' \
  --js='!core/block_render_svg_vertical.js' \
  ...
  --entry_point=Blockly \
  --js_output_file blockly_compressed_horizontal.js

if [ -s blockly_compressed_horizontal.js ]; then
  echo Compilation OK.
else
